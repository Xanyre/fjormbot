...
YoutubeTrack.getInfoFromVid = function(vid, m, cb) {
  ...
  ytdl.getInfo(requestUrl, (err, info) => {
    if (err) cb(err, undefined);
      ...
      video.username = m.author.username;
...
YoutubeTrack.prototype.prettyPrint = function() {
  return `**${this.title}** by **${this.author.name}** *(${this.formatViewCount()} views)* [${this.formatTime()}]`;
};

YoutubeTrack.prototype.fullPrint = function() {
  return `${this.prettyPrint()}, _**added by ${this.username}**_`;
};
